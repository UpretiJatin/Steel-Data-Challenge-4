-- Create database 'Steel_Challenge4'
create database Steel_Challenge4;

-- Make database as default
use Steel_Challenge4;
...
select concat(FirstName, " ",LastName) as Name_of_Customers, city from customers
where city = "New York";


/*Question 2. What is the total number of accounts in the Accounts table?*/

select count(*) as Total_number_of_accounts from Accounts;
...
select sum(a.balance) as Total_balance from
Accounts a inner join customers c using(customerId)
where c.city = "Los Angeles"; 


/*Question 5. Which branch has the highest average account balance?*/

select b.branchname, round(avg(a.balance),2) as Average_balance from
Accounts a inner join Branches b using(branchId)
...
select concat(c.firstname, " ", c.lastname) as Name_of_Customer, sum(a.balance) as Highest_balance
...
select concat(c.firstname, " ", c.lastname) as Name_of_Customer, count(t.transactionId) as No_of_Transactions from
customers c inner join accounts a using(customerId)
inner join transactions t using(accountId)
group by Name_of_Customer
...
select b.branchName, sum(a.balance) as Total_Balance from
branches b inner join Accounts a using(branchId)
group by b.branchname
order by sum(a.balance) desc limit 1;
...
select concat(c.firstname, " ", c.lastname) as Name_of_Customer, sum(a.balance) from
customers c inner join Accounts a using(customerId)
group by Name_of_Customer
order by sum(a.balance) desc limit 1;
...
select b.branchName, count(t.transactionId) as No_of_Transactions from
branches b inner join accounts a using(branchId)
inner join transactions t using(accountId)
group by b.branchname
order by count(t.transactionId) desc limit 2;